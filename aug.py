...
class TUDataset_aug(InMemoryDataset):
    ...
    def __init__(self, root, name, transform=None, pre_transform=None,
                 ...
                 cleaned=False, aug=None, rho=0.9):
        self.name = name
    ...
    def raw_file_names(self):
        ...
        return ['PATH_TO_DATASETS/TUDatasets/{}/{}/raw/{}_{}.txt'.format(self.name, self.name, self.name, name) for name in names]
    ...
    def get(self, idx):
        ...
        if self.aug == 'drop_ra':
            ...
            data_aug = drop_nodes_prob(deepcopy(data), nodes_score, self.rho)
...
def drop_nodes_prob(data, node_score, rho):
    ...
    if len(edge_idx) == 0:
    # Handle empty edge_idx by creating an empty tensor with shape [2, 0]
        data.edge_index = torch.empty((2, 0), dtype=torch.long)
    else:
    # Convert to PyTorch tensor and transpose
        data.edge_index = torch.tensor(edge_idx).transpose(0, 1)
