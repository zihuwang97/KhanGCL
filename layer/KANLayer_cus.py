...
class KANLayer_cus(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=2, num=5, k=3, 
                 ...
                 scale_sp=1.0, base_fun=torch.nn.SiLU(), 
                 grid_eps=0.02, grid_range=[-1, 1], 
                 sp_trainable=True, sb_trainable=True, 
                 save_plot_data = True, device='cpu', sparse_init=False,
                 return_y = True, neuron_fun='sum', use_base =True, update_grid=False):
        ''''
        ...
        '''
        super(KANLayer_cus, self).__init__()
        # size 
    ...
    def to(self, device):
        super(KANLayer_cus, self).to(device)
        self.device = device    
        return self
    ...
    def forward(self, x):
        ...
        if self.neuron_fun == "sum":
            ...
            raise ValueError("Unsupported function. Choose from 'sum', 'mean'.")
    ...
    def get_subset(self, in_id, out_id):
        ...
        (2, 3)
        '''
        spb = KANLayer_cus(len(in_id), len(out_id), self.num, self.k, base_fun=self.base_fun)
