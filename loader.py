...
allowable_features = {
    ...
    'possible_bonds' : [
        ...
        Chem.rdchem.BondType.DATIVE  # 确保包含 DATIVE
    ],
...
# class MoleculeDataset_aug_rgcl(InMemoryDataset):
#     def __init__(self,
#                  root,
#                  transform=None,
#                  pre_transform=None,
...
#         super(MoleculeDataset_aug_rgcl, self).__init__(root, transform, pre_transform, pre_filter)
#         self.transform, self.pre_transform, self.pre_filter = transform, pre_transform, pre_filter

#         if not empty:
#             self._data, self.slices = torch.load(self.processed_paths[0])

#         self.node_score = torch.zeros([self._data['x'].size(0)], dtype=torch.half)
...
#         elif self.aug == 'dropN_random':
#             nodes_score = self.node_score[self.slices['x'][idx]:self.slices['x'][idx + 1]]
#             data = drop_nodes_prob_random(data, self.low, self.high, nodes_score)


#         elif self.aug == 'dropN_auto':
#             nodes_score = self.node_score[self.slices['x'][idx]:self.slices['x'][idx + 1]]
#             data = drop_nodes_prob_auto(data, nodes_score)
...
def drop_nodes_prob_random(data, low, high, node_score):

    aug_ratio =  torch.rand(1).item()*(high-low) + low
    ...
    # 计算被删除节点的总概率值
    drop_prob_sum = node_prob[list(idx_drop_set)].sum()
    # 计算原始图的总概率值
    total_prob_sum = node_prob.sum()
    # 计算增强强度
    augmentation_strength = drop_prob_sum / total_prob_sum
    ...
    try:
        ...
        data.idx_nondrop = idx_nondrop  # Add the augmentation strength to the data object
        data.node_prob = node_prob
        data.augmentation_strength = augmentation_strength
    except:
        data = data
...
def drop_nodes_prob_auto(data, node_score):
    node_num, _ = data.x.size()
    _, edge_num = data.edge_index.size()
    ...
    random_probs = np.random.rand(node_num)
    idx_nondrop = np.where(random_probs < node_prob)[0]
    idx_drop_set = set(np.setdiff1d(np.arange(node_num), idx_nondrop).tolist())
